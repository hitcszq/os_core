...
rtm_0x70_interrupt_handle:                  ;实时时钟中断处理过程
        ...
        ;zhangqi start下面完成了一个两个队列的任务调度，就绪队列和阻塞队列都是基于简单的先进先出
        inc byte [num]
        cmp byte [num],0x0f                   ;到0x0f时刻，跳到.z0处，将当前任务阻塞
        jnz .sta                               ;不是0f时刻，跳到.sta处进行下一个判断
         mov eax,tcb_chain                  
  .z0:                                      ;EAX=链表头或当前TCB线性地址
         mov ebx,[eax]                      ;EBX=下一个TCB线性地址
         or ebx,ebx
         jz .irtn                           ;链表为空，或已到末尾，从中断返回
         cmp word [ebx+0x04],0xffff         ;是忙任务（当前任务）跳到.z1处，
         je .z1
  ...
  .z1:
         ...
         mov edx,[tcb_busy]
         cmp edx,0x00000000          ;阻塞队列是否为空
         jnz .z2                            ;阻塞队列不为空，跳到.Z2,找到阻塞队列的最后一项，链上去
         mov [tcb_busy],ebx                 ;阻塞队列为空，则直接链接到阻塞队列的头指针
         mov dword [ebx],0x00000000
        ; mov word [ebx+0x04],0x0000
        ; mov word [eax+0x04],0xffff
         jmp .de                             ;阻塞完成，跳到.de处从就绪队列中找到新任务去执行
  
  .z2:   
        mov eax,[tcb_busy]
  .z4:  
        mov ecx,[eax]
        or ecx,ecx          ;判断指针是否为空,空则说明是队尾，到.z3,将当前阻塞的tcb加到其后
        jz  .z3                   
        mov eax,[eax]       ;指向下一个TCB，
        jmp .z4             ;
    
  .z3:
        mov [eax],ebx                         ;将要阻塞的任务添加到阻塞队列的队尾
        mov dword [ebx],0x00000000                ;标记为队尾
        jmp  .de                            ;阻塞完成，跳到.de处从就绪队列中找到新任务去执行
  
  .sta:  
        cmp byte [num],0x1f                 ;判断现在是否是0x1f时刻,若也不是，跳到.re处，按照原来的方法执行就绪队列中任务的切换
        jnz .re                             ;若是，则将阻塞队列的第一个阻塞任务抽出，加到就绪队列中
        mov byte [num],0x00                 ;将时刻标志位清零
        mov edx,[tcb_busy]
       cmp edx,0x00000000     ;阻塞队列是否为空，若是，则跳到.re处，按照原来的方法执行就绪队列中任务的切换
        je .re
        mov edx,[tcb_busy]                  ;若不是,将阻塞队首的任务拿出
        mov eax,[edx]
        mov [tcb_busy],eax                    ;[tcb_busy]队首任务被拿出，队首指针指向下一个任务
        
        mov eax,[tcb_chain]                    ;将拿出的任务添加到就绪队列的队尾
  .j4:  
        mov ecx,[eax]
        or ecx,ecx          ;判断指针是否为空,空则说明是队尾，到.j3,将当前拿出的tcb加到其后
        jz  .j3                   
        mov eax,[eax]       ;指向下一个TCB，
        jmp .j4   
  .j3:
        mov [eax],edx                         ;将要拿出的任务添加到就绪队列的队尾
        mov dword [edx],0x00000000                ;标记为队尾
        jmp  .re                            ;解除阻塞完成，跳到.re处从就绪队列中找到新任务去执行
